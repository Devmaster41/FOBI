...
if __name__=="__main__":
    ...
    # ---------------------------------------------------------------------------
    # ===========================================================#
    # Modified path , Last update : 19/12/2563
    # Wipop Panyatipsakul, FRAB7 FIBO KMUTT
    # This path is for FOBI show with camera. Take guests' a photo and give it as souvenir.
    #
    # README-README-README-README-README-README-README-README-README-README-README
    # Run with python 2
    # There're 3 modes of proper photo, [none, hatch, gpen]
    # If use 'none' -> comment '#' at camera.exposure_mode
    # If use hatch,gpen -> enable camera.exposure_mode
    # ===========================================================#

    # runtime dependency on Python Imaging Library

    def pz(x):  # this is for timestamp
        if x < 10:
            return "0" + str(x)
        else:
            return x
    ...
    def welcome_F1():
        from PIL import Image
        fi = Image.open("fobi Asset 1.png")
        data = list(fi.getdata())
        w, h = fi.size
        p.justify("C")
        ...
        nowt = datetime.now()
        nowtt = (
            "{:d}-{}-{} {}:{}:{}".format(nowt.year, pz(nowt.month), pz(nowt.day), pz(nowt.hour), pz(nowt.minute),
                                         pz(nowt.second)))
        print("-----------------------------------")
        ...
        p.print_text("\nWelcome to ")
        p.bold()
        p.print_text("HCI Lab")
        p.bold(False)
        p.print_text(".")
        p.justify("")
        p.print_text("\nFIBO Tour at Suankularb.\n")

    def cap_print_therm():
        from PIL import Image
        camera = PiCamera()

        print("On process....")  # Flash Start

        camera.rotation = 0
        # camera.color_effects = (128, 128)  # turn camera to black and white
        camera.resolution = (384, 200)  # or 1920 x 1080,2592 x 1944 ,etc.
        camera.brightness = 63  # 0-100 , 50=default
        camera.annotate_background = Color('black')
        camera.annotate_foreground = Color('white')
        camera.annotate_text_size = 20  # text size 6-160 , 32=default
        camera.annotate_text = "   Xhibition !   "  # fill text in pic

        camera.image_effect = 'none' # 'gpen''hatch'
        camera.exposure_mode =  'nightpreview' # 'antishake'
        time.sleep(1)

        nt = datetime.now()
        # camera.capture('/home/pi/Desktop/img{:d}.{:d}.{:d}.{:d}.{:d}.{:d}.jpg' \
        #               .format(nt.year, nt.month, nt.day, nt.hour, nt.minute, nt.second))  # capture
        camera.capture('/home/pi/py-thermal-printer/capture.png')  # capture
        print("Finish !")  # Flash Finish

        pci = Image.open("capture.png")
        data = list(pci.getdata())
        w, h = pci.size
        p.print_bitmap(data, w, h, True)

    def souvenir_F1():  # welcome_F1() + cap_print_therm()
        from PIL import Image
        # Photo part setting --------------------------------------
        camera = PiCamera()

        print("On process....")  # Flash Start here

        camera.rotation = 0
        camera.resolution = (384, 400)  # or 1920 x 1080,2592 x 1944 ,etc.
        camera.brightness = 63  # 0-100 , 50=default
        ...
        mode = 'hatch'  #'gpen''hatch'  CHOOSE MODE HERE++++++++++++++++++++++++++++++

        camera.image_effect = mode
        #camera.exposure_mode = 'antishake'
        if mode == 'hatch' or mode == 'gpen':
            camera.exposure_mode = 'antishake'  # or 'backlight'
            time.sleep(1)
        camera.capture('/home/pi/py-thermal-printer/capture.png')  # capture
        print("capture Finish !")  # Flash Finish here

        # Start printing-------TEXT part-----------------------------------------------
        print("printing....")
        fi = Image.open("fobi Asset 1.png")
        data = list(fi.getdata())
        w, h = fi.size
